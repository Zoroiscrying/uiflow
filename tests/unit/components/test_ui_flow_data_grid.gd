...
func test_sort_by_descending() -> void:
	_grid.add_column("Name", 100, true)
	_grid.add_column("Level", 80, true)
	_grid.set_data([["Mage", "3"], ["Warrior", "5"], ["Rogue", "7"]])
	_grid.sort_by(1, false)
	var sorted := _grid.get_data()
	assert_that(sorted[0][1]).is_equal("7")
	assert_that(sorted[2][1]).is_equal("3")
...
## Test: column definition stores properties
func test_column_properties() -> void:
	_grid.add_column("Test", 150, true)
	var col: UIFlowDataGrid.Column = _grid._columns[0]
