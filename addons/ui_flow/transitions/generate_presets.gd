...
func _run() -> void:
	...
	# Special effects
	_save("bounce_in", _make_scale(Vector2.ZERO, Vector2.ONE, 0.5, Tween.EASE_OUT, Tween.TRANS_ELASTIC))
	_save("zoom_in", _make_scale(Vector2(3, 3), Vector2.ONE, 0.3, Tween.EASE_OUT))
