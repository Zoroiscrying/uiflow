## Survivors HUD — dynamic health/mana/XP bars with animated transitions.
...
func show_wave_start(wave: int) -> void:
	...
	# Fade out wave label after 2 seconds
	var tween: Tween = create_tween()
...
func show_damage_number(amount: int, world_pos: Vector3) -> void:
	...
	# Add to page container (not self) so it's positioned in viewport space
