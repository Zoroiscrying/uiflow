...
var _page_script: GDScript


func _can_begin(object: Object) -> bool:
	# Only show for UIFlowPage nodes
	if object is Control:
		var script = object.get_script()
		if script and script.get_global_name() == "UIFlowPage":
			return true
		# Check if it extends UIFlowPage
		while script:
			if script.get_global_name() == "UIFlowPage":
				return true
...
func _check_method_override(script: GDScript, method_name: String) -> bool:
	# Check if the script overrides this method
	var base_script = script.get_base_script()
	if base_script == null:
		return false

	# Get the script's source to check for method definition
	var source := script.source_code
	if source == null:
		return false

	# Simple check: look for "func method_name" in the source
