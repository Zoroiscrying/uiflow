...
Godot 4.6 在方向输入时会按显式 `focus_neighbor_*` 设置或全视口几何猜测来移动焦点——但这个猜测不了解你的 UI 结构，可能跳到被覆盖的下层页面或无关的悬浮层上。`UIFlow.Focus` 在引擎处理之前拦截方向输入，为顶层页面提供权威导航：

- `ui_left` / `ui_right` / `ui_up` / `ui_down`（方向键、十字键、左摇杆）只在**顶层页面内**的可聚焦控件间移动焦点——焦点不会泄漏到下层页面。
- 控件上显式设置的 `focus_neighbor_*` 优先于自动几何搜索。
- 禁用和不可见的控件会被自动跳过。
- 内部使用方向键的控件（LineEdit、TextEdit、ItemList、Tree、滑条、标签页、滚动容器）保留自己的按键；焦点持有者在顶层页面之外时（如悬浮对话框）交由引擎处理。
